...
export default function IdeaDetailPage() {
    ...
    if (loading) return <div className="p-8 text-muted">Syncing with cortex...</div>;
    if (!idea) return <div className="p-8 text-muted">Idea not found in memory.</div>;
            ...
            <div className="px-4">
                ...
                {tab === "provenance" && (
                    <div className="space-y-4">
                        <p className="text-sm text-muted mb-4">Raw inputs that have been attached to this idea by the Cognitive Engine.</p>
                        {fragments.length === 0 ? (
                            <div className="text-muted italic">No direct fragments linked (Genesis might differ).</div>
                        ) : (
                            fragments.map(f => (
