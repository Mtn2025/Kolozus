...
export const api = {
    ...
    deleteSpace: (id: string) => fetchJson(`/spaces/${id}`, { method: "DELETE" }),
    ...
    getVectorStats: () => fetchJson<{ total_embeddings: number; spaces_breakdown: Record<string, number> }>("/stats/vectors"),
    getEntropy: () => fetchJson<{ cognitive_drift: number; stability_percentage: number; germinal_ideas: number; consolidated_ideas: number }>("/stats/entropy"),
    getCounts: () => fetchJson<{ spaces: number; products: number; fragments: number; decisions_logged: number }>("/stats/counts"),
    getAuditLogs: (limit: number = 50) => fetchJson<any[]>(`/audit/logs?limit=${limit}`)
};
